...
function enqueue_admin_assets() {
	wp_enqueue_script(
		...
		plugin_dir_url( __DIR__ ) . '/assets/admin-actions.js',
		array( 'jquery' ),
...
function add_webhook_settings_script() {
	...
	jQuery(document).ready(function($) {
		...
		if ($('#woocommerce_axytoswc_webhook_api_key').length) {
			...
			var $generateBtn = $('<button type="button" class="button button-primary" style="margin-left: 10px;">
			<?php
			echo esc_js(
			...
			var $infoBtn = $('<button type="button" class="button axytos-info-btn" style="margin-left: 10px; background: #6c757d; border-color: #6c757d; color: white;">
			<?php
			echo esc_js(
			...
			$generateBtn.on('click', function(e) {
				...
				// Show confirmation
				var $notice = $('<div class="notice notice-success inline" style="margin: 10px 0;"><p>
				<?php
				echo esc_js(
					__(
		...
		<div class="card">
			...
			<table class="form-table" style="margin-top: 0;">
				...
				<tr>
					...
					<td style="padding: 8px 10px;">
						<?php
						if ( $next_scheduled['process_pending'] ) {
							echo wp_date(
								get_option( 'date_format' ) . ' ' . get_option( 'time_format' ),
											...
											<div style="margin: 3px 0;">
												...
												<?php if ( ! empty( $retry_item['fail_reason'] ) ) : ?>
													<button type="button" class="button button-small axytos-error-details-btn"
															...
															data-errors="<?php echo esc_attr( json_encode( array( $retry_item ) ) ); ?>"
										...
										<?php foreach ( $broken_list as $broken_item ) : ?>
											<div style="margin: 3px 0;">
												...
												<?php if ( ! empty( $broken_item['fail_reason'] ) ) : ?>
													<button type="button" class="button button-small axytos-error-details-btn"
															data-order-id="<?php echo esc_attr( $order_id ); ?>"
															data-errors="<?php echo esc_attr( json_encode( array( $broken_item ) ) ); ?>"
															style="background: #6c757d; color: white; border-color: #6c757d; margin-left: 5px; font-size: 10px; padding: 2px 6px;">
														<?php echo __( 'View Error', 'axytos-wc' ); ?>
													</button>
												<?php endif; ?>
