...
class AxytosWebhookHandler
{
     ...
     */
    public function authenticate_webhook_request($request)
    {
        ...
        if (empty($this->webhook_api_key)) {
            $this->log_webhook_activity('ERROR', 'Webhook API key not configured', [
                'ip' => $client_ip,
                'user_agent' => $request->get_header('user-agent')
            ]);
            return new \WP_Error(
        ...
        if (!$content_type || !in_array($content_type['value'], ['application/json', 'text/plain'])) {
        ...
        if (empty($provided_key)) {
            ...
            $this->log_webhook_activity('ERROR', 'Missing webhook API key in request', [
                'ip' => $client_ip,
                'user_agent' => $request->get_header('user-agent')
            ]);
            return new \WP_Error(
                'missing_api_key',
                __('API key is required in X-Axytos-Webhook-Key header.', 'axytos-wc'),
                ['status' => 401]
            );
        }
        ...
        if (!hash_equals($this->webhook_api_key, $provided_key)) {
            $this->record_failed_attempt($client_ip);
            $this->log_webhook_activity('ERROR', 'Invalid webhook API key provided', [
                'ip' => $client_ip,
                'user_agent' => $request->get_header('user-agent'),
    ...
    public function handle_webhook_request($request)
    {
        ...
        try {
            ...
            if (strlen($raw_body) > 10240) { // 10KB limit
            ...
            if (!$order) {
                $error_message = sprintf(__('Order with ID %d not found.', 'axytos-wc'), $order_id);
                $this->log_webhook_activity('ERROR', $error_message, ['order_id' => $order_id]);
            ...
            if (!empty($curr_status) && $current_order_status !== $curr_status) {
                $warning_message = sprintf(
                    __('Current order status (%s) does not match expected status (%s). Update may be outdated.', 'axytos-wc'),
            ...
            $this->process_order_update($order, $new_status, $invoice_number, $tracking_number);

            $success_message = sprintf(__('Order %d updated successfully.', 'axytos-wc'), $order_id);
     ...
     */
    private function process_order_update($order, $new_status, $invoice_number = null, $tracking_number = null)
    {
        ...
        $note_parts[] = sprintf(__('ERP updated status to: %s', 'axytos-wc'), $new_status);
        
        if (!empty($invoice_number)) {
            $note_parts[] = sprintf(__('Invoice: %s', 'axytos-wc'), $invoice_number);
            $order->update_meta_data('_axytos_erp_invoice_number', sanitize_text_field($invoice_number));
        }
        
        if (!empty($tracking_number)) {
            $note_parts[] = sprintf(__('Tracking: %s', 'axytos-wc'), $tracking_number);
            $order->update_meta_data('_axytos_erp_tracking_number', sanitize_text_field($tracking_number));
        }
        
        // Add timestamp for the ERP update
        $order->update_meta_data('_axytos_erp_last_update', current_time('mysql'));
        ...
        if ($wc_status && $wc_status !== $order->get_status()) {
            $status_note = sprintf(
                __('Order status updated from %s to %s via ERP webhook.', 'axytos-wc'),
                $order->get_status(),
                $wc_status
            );
    ...
    private function map_erp_status_to_wc_status($erp_status)
    {
        ...
        $status_mapping = apply_filters('axytos_webhook_status_mapping', $status_mapping);
        
        return isset($status_mapping[$erp_status_lower]) ? $status_mapping[$erp_status_lower] : null;
    }
     ...
     */
    private function log_webhook_activity($level, $message, $context = [])
    {
        ...
        switch (strtoupper($level)) {
            case 'ERROR':
                $this->logger->error($log_message, ['source' => 'axytos-webhook']);
                break;
            case 'WARNING':
                $this->logger->warning($log_message, ['source' => 'axytos-webhook']);
                break;
            case 'INFO':
            default:
                $this->logger->info($log_message, ['source' => 'axytos-webhook']);
                break;
        }
    }
