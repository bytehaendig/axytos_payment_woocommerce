...
class AxytosApiClientTest extends WP_UnitTestCase {
	...
	public function apiEndpointProvider() {
		return array(
			array( 'invoicePrecheck', '/Payments/invoice/order/precheck', 'POST' ),
			array( 'orderConfirm', '/Payments/invoice/order/confirm', 'POST' ),
			array( 'updateShippingStatus', '/Payments/invoice/order/reportshipping', 'POST' ),
			array( 'returnItems', '/Payments/invoice/order/return', 'POST' ),
			array( 'refundOrder', '/Payments/invoice/order/refund', 'POST' ),
			array( 'createInvoice', '/Payments/invoice/order/createInvoice', 'POST' ),
		);
	}
	...
	public function getEndpointProvider() {
		return array(
			array( 'getPaymentStatus', '/Payments/invoice/order/paymentstate/12345', 'GET', '12345', '{"status": "paid"}' ),
			array( 'getAgreement', '/StaticContent/creditcheckagreement', 'GET', null, '{"agreement": "terms and conditions"}' ),
		);
	}
	...
	public function test_reverse_cancel_order_with_request_body() {
		...
		$expectedData = array( 'externalOrderId' => $orderId );
	...
	public function test_http_error_handling( $statusCode, $statusText ) {
		...
		try {
			$client->invoicePrecheck( array( 'test' => 'data' ) );
	...
	public function test_successful_status_codes() {
		$successCodes = array( 200, 201, 299 );

		foreach ( $successCodes as $code ) {
			$this->mockHttpRequest( $code, '{"success": true}' );
			$client = new AxytosApiClient( $this->testApiKey );

			$result = $client->invoicePrecheck( array( 'test' => 'data' ) );
			$this->assertEquals( '{"success": true}', $result );
		}
	}
	...
	public function test_empty_request_data() {
		...
		$result = $client->invoicePrecheck( array() );
		$this->assertEquals( '{"success": true}', $result );
	}
